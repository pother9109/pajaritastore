...
function buildVariant(row, colorLookup) {
  ...
  const productType = normalizeKey(row.Tipo || 'normal');
  const isComingSoon = productType === 'proximamente';
  const price = isComingSoon ? null : toNumber(row.precio_venta);
  const compareCandidate = isComingSoon ? null : toNumber(row.precio_lista);
  const compareAtPrice = compareCandidate > price ? compareCandidate : null;

  const images = parseImageList(row['URL imagen']);
...
function buildProductGroup(rows, colorLookup) {
  const first = rows[0];
  const productType = normalizeKey(first.Tipo || 'normal');
  const isComingSoon = productType === 'proximamente';
  ...
  const price = isComingSoon ? null : getPriceRange(rows, 'precio_venta');
  const compareCandidate = isComingSoon ? null : getPriceRange(rows, 'precio_lista');
  const compareAtPrice = compareCandidate > price ? compareCandidate : null;
...
export async function getProducts() {
  ...
  const filteredRows = catalogRows.filter((row) => {
    ...
    if (type === 'normal' && inventory < 1) return false;
    return type === 'normal' || type === 'proximamente';
  });
